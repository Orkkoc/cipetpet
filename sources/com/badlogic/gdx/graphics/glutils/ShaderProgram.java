...
public class ShaderProgram implements Disposable {
    ...
    public void setUniform1fv(String name, float[] values, int offset, int length) {
        ...
        BufferUtils.copy(values, (Buffer) this.floatBuffer, length, offset);
    ...
    public void setUniformMatrix(String name, Matrix4 matrix2, boolean transpose) {
        ...
        int location = fetchUniformLocation(name);
        this.matrix.clear();
        BufferUtils.copy(matrix2.val, (Buffer) this.matrix, matrix2.val.length, 0);
    ...
    public void setUniformMatrix(String name, Matrix3 matrix2, boolean transpose) {
        ...
        checkManaged();
        int location = fetchUniformLocation(name);
        float[] vals = matrix2.getValues();
        this.matrix.clear();
        BufferUtils.copy(vals, (Buffer) this.matrix, vals.length, 0);
    ...
    public void setUniformMatrix4fv(String name, FloatBuffer buffer2, int count, boolean transpose) {
        GL20 gl = Gdx.graphics.getGL20();
        checkManaged();
        buffer2.position(0);
        gl.glUniformMatrix4fv(fetchUniformLocation(name), count, transpose, buffer2);
    }

    public void setUniformf(String name, Vector2 values) {
        setUniformf(name, values.f165x, values.f166y);
    }

    public void setUniformf(int location, Vector2 values) {
        setUniformf(location, values.f165x, values.f166y);
    }

    public void setUniformf(String name, Vector3 values) {
        setUniformf(name, values.f170x, values.f171y, values.f172z);
    }

    public void setUniformf(int location, Vector3 values) {
        setUniformf(location, values.f170x, values.f171y, values.f172z);
    }

    public void setUniformf(String name, Color values) {
        setUniformf(name, values.f70r, values.f69g, values.f68b, values.f67a);
    }

    public void setUniformf(int location, Color values) {
        setUniformf(location, values.f70r, values.f69g, values.f68b, values.f67a);
    }

    public void setVertexAttribute(String name, int size, int type2, boolean normalize, int stride, Buffer buffer2) {
        GL20 gl = Gdx.graphics.getGL20();
        checkManaged();
        int location = fetchAttributeLocation(name);
        if (location != -1) {
            gl.glVertexAttribPointer(location, size, type2, normalize, stride, buffer2);
        }
    }

    public void setVertexAttribute(int location, int size, int type2, boolean normalize, int stride, Buffer buffer2) {
        GL20 gl = Gdx.graphics.getGL20();
        checkManaged();
        gl.glVertexAttribPointer(location, size, type2, normalize, stride, buffer2);
    }
    ...
    public static String getManagedStatus() {
        ...
        while (i$.hasNext()) {
            builder.append(shaders.get(i$.next()).size());
            builder.append(" ");
        }
        builder.append("}");
    ...
    public int getAttributeType(String name) {
        int type2 = this.attributeTypes.get(name, -1);
        if (type2 == -1) {
            return 0;
        }
        return type2;
    ...
    public int getUniformType(String name) {
        int type2 = this.uniformTypes.get(name, -1);
        if (type2 == -1) {
            return 0;
        }
        return type2;
