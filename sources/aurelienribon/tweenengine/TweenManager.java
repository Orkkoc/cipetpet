...
public class TweenManager {
    ...
    public boolean containsTarget(Object target) {
        ...
        for (int i = 0; i < n; i++) {
            if (this.objects.get(i).containsTarget(target)) {
                return true;
            }
        }
        return false;
    ...
    public boolean containsTarget(Object target, int tweenType) {
        int n = this.objects.size();
        for (int i = 0; i < n; i++) {
            if (this.objects.get(i).containsTarget(target, tweenType)) {
                return true;
            }
        }
        return false;
    ...
    public void killTarget(Object target) {
        int n = this.objects.size();
        for (int i = 0; i < n; i++) {
            this.objects.get(i).killTarget(target);
        }
    }
    ...
    public void killTarget(Object target, int tweenType) {
        int n = this.objects.size();
        for (int i = 0; i < n; i++) {
            this.objects.get(i).killTarget(target, tweenType);
        }
    }
    ...
    public void update(float delta) {
        ...
        if (delta >= 0.0f) {
            ...
            for (int i2 = 0; i2 < n; i2++) {
                this.objects.get(i2).update(delta);
            }
            return;
        }
        for (int i3 = this.objects.size() - 1; i3 >= 0; i3--) {
            this.objects.get(i3).update(delta);
        }
    }
